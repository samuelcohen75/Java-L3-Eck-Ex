...
public class EE47MyClass {
	...
	/**
	 * Creates the arrays that hold the numbers for the questions and fills them
	 * with random numbers. The parameters are arrays that will hold the random
	 * numbers for the first and second operands of each addition problem. The
	 * arrays must have already been created when this subroutine is called!
	 */
	...
	/**
	 * Asks the user each of the ten quiz questions and gets the user's answers. The
	 * answers are stored in an array, which is created in this subroutine. The
	 * first two parameters hold the operands for the quiz questions. The user's
	...
	/**
	 * Shows all the questions, with their correct answers, and computes a grade for
	 * the quiz. For each question, the user is told whether they got it right. The
	 * first two parameters hold the operands for the quiz questions, and the third
	 * parameter holds the answers that the user gave to the quiz.
	 */
	private static void gradeQuiz(int[] firstNumbers, int[] secondNumbers, int[] userAnswers) {
		...
		for (int i = 0; i < 10; i++) {
			...
			System.out.printf("   Question %2d:  %2d + %2d  =  %2d.  ", questionNum, firstNumbers[i], secondNumbers[i],
					correctAnswer);
